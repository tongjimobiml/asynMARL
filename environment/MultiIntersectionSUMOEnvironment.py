...
class MultiIntersectionSUMOEnvironment(gym.Env):
	CONNECTION_LABEL = 0
	...
	def __init__(self, all_args):
		...
		self.label = str(MultiIntersectionSUMOEnvironment.CONNECTION_LABEL)
		...
		for agent in self.data_utils.regions:
			...
			self.region_graphs[agent] = self.data_utils.graph.copy()
	...
	def _start_simulation(self):
		...
		# sumo_cmd = [self._sumo_binary, '-n', self._network_file, '-r', self.current_route_file]
		# sumo_cmd = [self._sumo_binary, '-n', self._network_file, '-r']
	...
	def observe(self):
		...
		for road_id in self.data_utils.edge_info.keys():
			...
			# Update observable graph
			region = self.data_utils.node_dict[v_from]
			self.region_graphs[region][v_from][v_to]['travel_time'] = travel_time
			...
			if self.road_state[road_id] == 0:
				if vehicle_number_last_step > self.data_utils.capacity_limit[road_id]:
					...
					# traci.edge.setMaxSpeed(road_id, self.speed * self.congest_factor)
					traci.edge.setMaxSpeed(road_id, self.speed * self.congest_factor * self.data_utils.capacity_limit[road_id] / vehicle_number_last_step)
				...
				if self.sim_time - self.road_state[road_id] > self.congest_time:
					...
					# recover road state
					traci.edge.setMaxSpeed(road_id, self.speed)
		...
		for agent in self._agents:
			...
			for _, action_road in self.action_map[agent].items():
				...
				# 更改edge_feature数据结构为[[road1], [road2], ..., [road3]]
	...
	def get_queries(self):
		...
		for cveh in current_vehicles:
			...
			if road_id == self.vehicles[cveh]['destination']:
				...
				# Check if the vehicle need next road action
				if road_id == self.vehicles[cveh]['intra_target'] and road_id != self.vehicles[cveh]['route'][-1]:
				...
				if road_id != self.vehicles[cveh]['route'][-1]:
					self.vehicles[cveh]['route'].append(road_id)
				# 	self.sumo.vehicle.rerouteTraveltime(cveh)
	...
	###########################################################################
	# TESTING POLICY: SPFWR
	def get_spfwr_action(self, queries):
		actions = {}
		for query in queries:
			region = query['region']
			dest_node = self.data_utils.edge_info[query['destination']]['from']
			if query['node'] == dest_node:
				actions[query['vehicle']] = query['destination']
			else:
				sp_path = nx.shortest_path(self.region_graphs[region], query['node'], dest_node,
										   weight='travel_time')
				actions[query['vehicle']] = self.data_utils.graph[sp_path[0]][sp_path[1]]['id']
		return actions
