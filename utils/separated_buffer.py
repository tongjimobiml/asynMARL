...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.use_query_rnn = args.use_query_rnn
        ...
        self.share_obs = []
        self.obs = []
        # self.value_preds = [np.zeros((self.n_rollout_threads, 1), dtype=np.float32)]
        # self.next_value_preds = [np.zeros((self.n_rollout_threads, 1), dtype=np.float32)]
        self.value_preds = []
        self.next_value_preds = []
        # self.available_actions = [np.ones((1, self.n_rollout_threads, act_space.n), dtype=np.float32)]
        ...
        self.rewards = []
        self.masks = [np.ones((self.n_rollout_threads, 1), dtype=np.float32)]
        self.bad_masks = []
        self.active_masks = []

        self.step = 0

    def insert(self, share_obs, obs, query_obs, actions, action_log_probs,
               value_preds, next_value_preds, rewards, masks, bad_masks=None, active_masks=None,
               available_actions=None):
        # todo: 增加obs_road和obs_query
        ...
        self.query_obs.append(query_obs.copy())
        ...
        self.next_value_preds.append(next_value_preds.copy())
        self.rewards.append(rewards.copy())
        self.masks.append(masks.copy())
        self.available_actions.append(available_actions.copy())
        self.step = (self.step + 1) % self.episode_length
    ...
    def before_update(self):
        ...
        num_tuples = len(self.rewards)
        self.share_obs = np.array(self.share_obs, dtype=np.float32)
        self.obs = np.array(self.obs, dtype=np.float32)
        self.query_obs = [np.array(self.query_obs[i], dtype=np.float32) for i in range(num_tuples)]
        self.masks = np.array(self.masks, dtype=np.float32)
        self.bad_masks = np.ones_like(self.masks)
        self.active_masks = np.ones_like(self.masks)
        self.rewards = np.array(self.rewards, dtype=np.float32).reshape((num_tuples, self.n_rollout_threads, 1))
        self.returns = np.zeros((num_tuples, self.n_rollout_threads, 1), dtype=np.float32)
        self.value_preds = np.array(self.value_preds, dtype=np.float32)
        self.next_value_preds = np.array(self.next_value_preds, dtype=np.float32)
        self.actions = np.array(self.actions, dtype=np.float32)
        self.action_log_probs = np.array(self.action_log_probs, dtype=np.float32)
        self.available_actions = np.array(self.available_actions, dtype=np.float32)
    ...
    def compute_returns(self, value_normalizer=None):
        ...
        for step in range(self.rewards.shape[0]):
            if self._use_valuenorm:
                self.returns[step] = value_normalizer.denormalize(self.next_value_preds[step]) * self.masks[step + 1] + self.rewards[step]
            else:
                self.returns[step] = self.next_value_preds[step] * self.masks[step + 1] + self.rewards[step]

            # delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(self.next_value_preds[step]) * self.masks[step + 1] - value_normalizer.denormalize(self.value_preds[step])
            # gae = delta
            # self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
            # self.returns[step] = value_normalizer.denormalize(self.next_value_preds[step]) + self.rewards[step]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        share_obs = self.share_obs.reshape(-1, *self.share_obs.shape[2:])
        ...
        for indices in sampler:
            ...
            # query_obs_batch: [np.array, ..., np.array] Tensor i:[num_steps, Dim]
            query_obs_batch = [self.query_obs[i] for i in indices]
            ...
            yield share_obs_batch, obs_batch, query_obs_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
